...
function getSA() {
    ...
    if (!raw)
        throw new Error('GOOGLE_VERTEX_SA_JSON is not set');
    _sa = JSON.parse(raw);
    return _sa;
}
...
async function getVertexToken() {
    ...
    if (!res.ok)
        throw new Error(`Vertex token error ${res.status}: ${await res.text()}`);
...
async function generateImage(prompt, options = {}) {
    ...
    const nbRatio = options.ratio === '4:5' ? '3:4' : options.ratio;
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
        ...
        if (res.status === 429) {
            console.error(`[vertex] 429, retry ${attempt + 1}/3`);
            continue;
        }
        if (!res.ok)
            throw new Error(`Vertex error ${res.status}: ${(await res.text()).slice(0, 400)}`);
        const data = await res.json();
...
async function searchWeb(query, includeImages = false) {
    ...
    if (!apiKey)
        return { text: 'Web search unavailable — TAVILY_API_KEY not set', imageUrls: [] };
    ...
    if (!res.ok)
        return { text: `Search failed (${res.status})`, imageUrls: [] };
    const data = await res.json();
...
// ─── MCP server ───────────────────────────────────────────────────────────────
const server = new Server({ name: 'contentflow', version: '1.0.0' }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === 'generate_image') {
            const { prompt, ratio, style } = args;
            const styleHint = style === 'professional' ? 'Studio product photograph — clean seamless background, controlled directional lighting, sharp focus.' :
            ...
            const ratioHint = ratio === '4:5' ? 'Portrait 4:5 framing.' :
        ...
        if (name === 'generate_carousel') {
            const { topic, slideCount = 5, platform = 'instagram', brandName, searchContext } = args;
            const anthropic = getAnthropic();
            const count = Math.max(2, Math.min(7, slideCount));
            const ratio = platform === 'linkedin' ? '1:1' : '4:5';
...
- imagePrompt: detailed NanoBanana image prompt (cinematic, specific scene, no text/logos)`;
            ...
            for (let i = 0; i < slides.length; i += 2) {
                ...
                const batchResults = await Promise.all(batch.map(slide => generateImage(`${slide.imagePrompt}\n\nCinematic editorial quality, ${ratio === '4:5' ? 'portrait 4:5' : 'square 1:1'} framing, premium brand aesthetic, no text overlays.`, { ratio }).catch(() => null)));
        ...
        if (name === 'generate_social_captions') {
            const { topic, platforms = ['instagram', 'twitter', 'linkedin'], tone = 'casual', brandName } = args;
            const anthropic = getAnthropic();
...
---`
        ...
        // ── search_web ────────────────────────────────────────────────────────────
        if (name === 'search_web') {
            const { query, include_images = false } = args;
        ...
        if (name === 'generate_ugc_brief') {
            const { product, platform = 'tiktok', duration = 30, tone = 'energetic' } = args;
            const anthropic = getAnthropic();
            const response = await anthropic.messages.create({
                model: 'claude-haiku-4-5',
                max_tokens: 1000,
