...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const msg = shots.length ? null : await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `PRODUCT SHEET:\nName: ${product.name}\nCategory: ${product.category ?? 'unknown'}\nDescription: ${product.description ?? ''}\n\nSHOTS NEEDED: ${count}\nAVOID (already shot): ${avoid.length ? avoid.join(' · ') : '(nothing yet)'}\n\n★ PRODUCT-PRESENT RULE — every concept MUST feature the ${product.name} product physically in the frame (held, poured, placed, worn, centred). NEVER draft a portrait / bust / atmosphere / mood shot where the product is absent or merely implied. If the concept can't fit the product in, redesign the concept.\n\n★ FILTER CONCEPTS ARE FINE — but scope them ★ Duotone / monochrome / tinted / high-contrast graphic treatments are welcome for their editorial punch. BUT the concept must EXPLICITLY state that the treatment applies only to the ENVIRONMENT/BACKGROUND — the hero product stays in full original colour. Write it like: "duotone blue treatment on the staircase and clouds, JAMU can remains in its original full-colour packaging as the single warm focal point." Never write a concept that implies the whole image (product included) is filtered.\n${sceneDetail ? `\n★ LOCATION LOCK — every single concept in this batch MUST happen INSIDE this specific location. Do NOT invent studio backdrops, pools, forests, or any other setting. All ${count} shots stay in the same location so a viewer scrolling through the set instantly reads them as one place shot at different moments/angles.\nLOCATION NAME: ${sceneDetail.name}\nLOCATION BRIEF: ${sceneDetail.scene_prompt}\nEvery concept prompt must explicitly reference elements of THIS location (its architecture, materials, palette, natural light, distinctive features from the brief above). Camera angle / composition / prop styling can vary between shots, but the ROOM/PLACE cannot change.\n` : ''}${coProducts.length ? `\nADDITIONAL PRODUCTS in the SAME shot (composed alongside the hero product, e.g. flat-lay of the range, held together, arranged as a set — treat them as the brand's related SKUs):\n${coProducts.map((cp, i) => `  ${i + 1}. ${cp.name}${cp.category ? ` (${cp.category})` : ''}`).join('\n')}\nEvery concept must feature ALL of these products together — never omit any. Do not describe their appearance; the image model receives their photos.` : ''}${influencersDetail.length ? (mode === 'ad'
    ...
    const sceneLockClause = sceneDetail
      ? `\n\n★ LOCATION LOCK — non-negotiable ★ This shot happens INSIDE ${sceneDetail.name}. ${sceneDetail.scene_prompt} The final image's environment (walls, floor, sky, plants, architecture, materials, palette, natural light direction) MUST match this location — the SCENE ANCHOR reference image shows exactly what the place looks like, mirror its lighting and materials. Do NOT invent a different backdrop, studio, forest, pool, or generic surface. The subject sits INSIDE this place.`
      : ''
    const productIntegrityClause = `\n\n★ HERO PRODUCT MUST APPEAR — absolute ★ The hero product (${product.name}) MUST be visibly present and clearly identifiable in this shot — held, poured, placed, worn, or centred in the composition. Do NOT return a shot without the product. Do NOT return a bust portrait, headshot, or pure atmosphere shot with no product. If the concept phrasing seems abstract, still include the product physically in the frame.

★ HERO PRODUCT INTEGRITY — absolute ★ Its packaging, label text, wordmark, illustration, colours, proportions and shape must match its reference photos EXACTLY, pixel-faithful. Do NOT redesign the label, do NOT change the type, do NOT alter the artwork, do NOT invent a new can/bottle shape.

★ PRODUCT COLOUR IMMUNITY — critical ★ Even if the concept applies a duotone / monochrome / sepia / tinted / filtered / high-contrast graphic treatment to the environment, the PRODUCT ITSELF must render in its ORIGINAL FULL COLOUR from its reference photos. The filter/tint stops at the product. Its label, illustration, wordmark and packaging colours stay accurate. Never apply the scene's colour treatment to the product surface. If the concept can't accommodate this (e.g. requires the product to be tinted blue), skip the treatment on the product and only apply it to the background/environment.`
    const peopleLockClause = influencersDetail.length
      ? `\n\n★ CAST LOCK — non-negotiable ★ Exactly ${influencersDetail.length} ${influencersDetail.length === 1 ? 'person' : 'people'} appear in this shot: ${influencersDetail.map((p, i) => `person ${i + 1} = the model shown in identity reference image${identityRefs.length > 1 ? 's' : ''}`).join(', ')}. No extra bodies, no phantom faces in the background, no crowd, no additional models introduced by the composition. If a body isn't clearly one of the referenced ${influencersDetail.length === 1 ? 'person' : 'people'}, it does not exist in this frame.`
      : `\n\n★ CAST LOCK ★ No people at all in this shot — product only. Do not add hands, silhouettes, or bodies unless they appear on the identity reference images (there are none).`

    // App/website products get different render prompts — the "product" is a
     // UI screenshot, not a physical object. Aesthetic mode composes the app
     // onto a laptop/phone screen in a scene; ad mode uses the UI itself as
     // the hero with typographic overlays. Branching lives inside the per-shot
     // closure below so it also picks up sceneLock / peopleLock clauses.
    const isAppProduct = product.product_type === 'app'
    const results = await Promise.allSettled(shots.map(sh => renderShot(() => {
      ...
      const styleRefPrompt = styleReference
        ...
        : null
      // App/website product prompt overrides. The "product" here is a UI
      // screenshot (landing page, dashboard, feature page). It has to render
      // AS a screen displayed inside a device — never as a physical object,
      // never as a printed poster.
      const appAestheticPrompt = influencer
        ? `${influencer.appearance_prompt}\n\nShot: ${sh.prompt}\n\nCinematic editorial lifestyle photograph. The recurring creator is candidly USING the app (${product.name}) — either on a modern laptop screen (MacBook Pro, thin-bezel, screen sharp and forward-facing to camera) or on a smartphone (held POV or in-hand three-quarter). The exact app UI from the reference screenshot(s) is displayed on the device screen crisply and legibly — do NOT redesign the interface, do NOT invent new UI elements, preserve the layout, typography, colours, logos and text of the reference screenshot exactly as pixel-faithful as possible. Real light, real shadows, real environment; the creator's attention is on the screen (or on the camera with the device visible). No text overlays, no watermarks, no plastic-CG feel.`
        : `${sh.prompt}\n\nCinematic editorial product photograph of the app (${product.name}) displayed on a modern device — a MacBook Pro / thin-bezel laptop on a desk, or an iPhone / smartphone held in a hand or resting on a surface. The exact app UI from the reference screenshot(s) is on-screen, crisp, legible, pixel-faithful to the reference — preserve layout, typography, colours, logos and text exactly, do NOT invent new UI. Composition is magazine-cover level: real materials (wood, marble, linen, glass), true directional lighting with real shadows, intentional colour palette, generous negative space. The device is the hero, the screen is the focal point. No text overlays, no watermarks, no logos beyond what's already inside the app UI.`
      const appAdPrompt = `${sh.prompt}\n\nCampaign-grade SaaS / app advertisement in the style of Linear, Vercel, Notion, Arc, Cash App, Duolingo. The HERO of this ad IS the app's UI screenshot from the reference images — treat the screenshot as the centrepiece artwork, tilted, floating, mocked into a laptop / phone, or laid flat as a poster hero. Preserve the UI EXACTLY as shown in the reference (layout, typography, colours, logos, text) — do NOT redesign the interface, do NOT invent screens.\n\nAround the UI, layer bold campaign typography: 2–5-word headline in a real modern typeface (ultra-bold condensed sans, high-contrast serif, refined display), a tiny sub-line or brand mark, real kerning and correct spelling — no gibberish. Typography integrates with the composition (wrapped around the device, cropped by the screen edge, sitting behind the UI mockup, huge behind a small floating iPhone). Confident negative space, one strong colour story per shot, dynamic off-centre composition. NEVER a plain dead-centre cutout with a floating headline.\n\nBackground: a rich intentional environment (gradient studio wash, gel-lit painted concrete, brand-coloured backdrop, on-desk photography with real material texture) — not a generic photoshop flat. Feel: Linear / Arc / Cash App launch-day energy — modern, restrained, confident. NOT a Fiverr banner, NOT a Canva template.${influencersDetail.length ? `\n\n★ ${influencersDetail.length === 1 ? 'PERSON' : `${influencersDetail.length} PEOPLE`} IN THE AD — the FIRST reference images define the exact model${influencersDetail.length > 1 ? 's' : ''}. Face(s) fully visible, matching the references, holding or interacting with the device that displays the app. Never crop a head, never let the device cover a face.` : ''}`
      const appProductIntegrityClause = `\n\n★ APP UI FIDELITY — absolute ★ The app UI shown in the reference screenshot(s) must render on-screen EXACTLY — pixel-faithful layout, exact typography, exact colours, exact wordmark/logo, exact text and buttons. Do NOT redesign the interface, do NOT invent new UI elements, do NOT swap the app for a generic mockup or a stock design. The UI belongs on a real device screen (laptop or phone) — never as a floating physical object, never as printed packaging. Its on-screen colours stay accurate even if the surrounding scene uses a tint / duotone treatment; the filter stops at the screen bezel.`
      const finalPrompt = (styleRefPrompt
        ? styleRefPrompt
        : isAppProduct
          ? (mode === 'ad' ? appAdPrompt : appAestheticPrompt)
          : (mode === 'ad'
            ? adPrompt
            : (influencer ? aestheticPromptWithInf : aestheticPrompt))) + coProductClause + sceneLockClause + (isAppProduct ? appProductIntegrityClause : productIntegrityClause) + peopleLockClause
      ...
      hintParts.push(isAppProduct
        ? `Image${productRefs.length > 1 ? 's' : ''} ${productStart}${productEnd > productStart ? `–${productEnd}` : ''}: HERO APP UI (${product.name}) — these are screenshots of the app / website. Render this exact UI on a device screen (laptop or phone) in the composition; preserve layout, typography, colours, logos and text pixel-faithfully. Never render as a physical object.`
        : `Image${productRefs.length > 1 ? 's' : ''} ${productStart}${productEnd > productStart ? `–${productEnd}` : ''}: HERO PRODUCT (${product.name}) — preserve packaging, label text, colours and shape exactly across all angles.`)
      idx = productEnd
