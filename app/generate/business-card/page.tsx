...
function CardPreview({ form, styleId, accentHex, logoSrc }: {
  form: FormData
  styleId: string
  accentHex: string
  logoSrc: string | null
}) {
  const dark = darken(accentHex, 50)
  ...
  if (styleId === 'minimal') {
    return (
      <div style={{ ...base, background: '#FFFFFF', boxShadow: '0 8px 40px rgba(0,0,0,0.12)', border: '1px solid #e8e8e8' }}>
  ...
  if (styleId === 'bold') {
    return (
      <div style={{ ...base, background: '#FFFFFF', boxShadow: '0 8px 40px rgba(0,0,0,0.12)' }}>
  ...
  if (styleId === 'dark') {
    return (
      <div style={{ ...base, background: '#0f0f0f', boxShadow: '0 8px 40px rgba(0,0,0,0.5)' }}>
        <div style={{ padding: '28px 32px', height: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <div>
  ...
  // gradient
  return (
    <div style={{ ...base, background: `linear-gradient(135deg, ${accentHex} 0%, ${dark} 100%)`, boxShadow: '0 8px 40px rgba(0,0,0,0.25)' }}>
      <div style={{ padding: '28px 32px', height: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <div>
...
export default function BusinessCardPage() {
  ...
  const canGenerate = form.name.trim().length >= 1 && creditBalance >= CREDIT_COST
  ...
  async function generate() {
    ...
    try {
      ...
      const logoBase64 = logoSrc ? logoSrc.split(',')[1] ?? null : null
      const logoMimeType = logoSrc ? logoSrc.split(';')[0].replace('data:', '') : null

      const res = await fetch('/api/business-card/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ form, styleId, accentColor: accentHex, logoBase64, logoMimeType }),
      })
      ...
      refreshCredits()
      showSuccess('Business card ready', 'Your card has been generated')
    } catch (err) {
    ...
    <main style={{ maxWidth: 1160, margin: '0 auto', padding: '42px 40px 90px' }} className="bc-page">
      <header style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 15, color: 'var(--ink-dim)', margin: '12px 0 0', maxWidth: 500, lineHeight: 1.55 }}>
          Fill in your details, pick a style, and generate a print-ready card image.
        </p>
      </header>

      <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 28, alignItems: 'start' }} className="bc-grid">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {([
              ...
              ['tagline', 'Tagline / Bio', 'Building products people love'],
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      background: active ? 'var(--ink)' : 'var(--bg-elev, var(--surface))',
                      border: `1px solid ${active ? 'var(--ink)' : 'var(--border)'}`,
                      color: active ? '#fff' : 'var(--ink-2, var(--ink-dim))',
          ...
          {/* Generate button */}
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <button
              onClick={generate}
              ...
              style={{
                ...
                background: !canGenerate || loading ? 'var(--ink-faint, #ccc)' : 'var(--ink)',
            ...
            >
              {loading ? (
                <>
                  <span style={{ display: 'inline-block', width: 14, height: 14, border: '2px solid rgba(255,255,255,0.4)', borderTopColor: '#fff', borderRadius: '50%', animation: 'bc-spin 0.8s linear infinite' }} />
                  Generating…
                </>
              ) : `Generate · ${CREDIT_COST} credits`}
            </button>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--ink-dim)' }}>
              <span>1 card · {CREDIT_COST} cr</span>
              <span>Balance: <strong style={{ color: creditBalance >= CREDIT_COST ? 'var(--good, #10b981)' : 'var(--danger, #e84a4a)' }}>{creditBalance}</strong></span>
            </div>
            {!form.name.trim() && (
              <p style={{ fontSize: 11.5, color: 'var(--ink-dim)', margin: 0, textAlign: 'center' }}>Enter a name to enable generation</p>
            )}
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: 28 }}>
            <div style={{ fontSize: 11, fontFamily: 'var(--font-mono)', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--ink-dim)', marginBottom: 16 }}>Live Preview</div>
            <div style={{ display: 'flex', justifyContent: 'center', padding: '8px 0' }}>
              <CardPreview form={form} styleId={styleId} accentHex={accentHex} logoSrc={logoSrc} />
            </div>
            <p style={{ fontSize: 11.5, color: 'var(--ink-dim)', textAlign: 'center', margin: '16px 0 0' }}>
              Preview is approximate · Generate for the final AI-rendered card
            </p>
          </div>
          ...
          {resultImage && (
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: 28 }}>
              <div style={{ fontSize: 11, fontFamily: 'var(--font-mono)', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--ink-dim)', marginBottom: 16 }}>Generated Card</div>
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 16 }}>
                <img
                  src={resultImage}
                  alt="Generated business card"
                  style={{ maxWidth: '100%', borderRadius: 10, boxShadow: '0 8px 40px rgba(0,0,0,0.14)', border: '1px solid var(--border)' }}
                />
              </div>
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <a
                  href={resultImage}
                  download={`business-card-${Date.now()}.png`}
                  style={{
                    display: 'inline-flex', alignItems: 'center', gap: 8,
          ...
          {loading && !resultImage && (
            <div style={{
              background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: 40,
              display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14,
            }}>
              <div style={{ width: 36, height: 36, border: '3px solid var(--border)', borderTopColor: 'var(--accent, #5B6BFF)', borderRadius: '50%', animation: 'bc-spin 0.8s linear infinite' }} />
              <p style={{ fontSize: 14, color: 'var(--ink-dim)', margin: 0 }}>Generating your business card…</p>
              <p style={{ fontSize: 12, color: 'var(--ink-dim)', margin: 0, opacity: 0.7 }}>This takes 10–15 seconds</p>
            </div>
          )}
        </div>
      </div>
