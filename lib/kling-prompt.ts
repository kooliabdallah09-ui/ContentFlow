...
6. Product handling MUST stay small and controlled. ${pronoun} keeps the product roughly where the start_image shows it. Micro-adjustments are allowed and often good: fingers shift grip, thumb runs along the label, small tilt so light catches the bottle, a subtle "presents" motion where the product moves a few centimetres toward the camera (SLOW — over most of the clip's duration, not a snap), a small "brings toward lens" beat where it drifts closer at half-frame speed. What is NEVER allowed: shakes, waves, gestures emphatically, thrusts, snaps toward the lens, or any move fast enough to cause motion blur on the product. Motion blur on the product = failure.
...
Words that imply performance for the camera and cue Kling to overact: excited, bubbly, energetic, animated, enthusiastic, expressive, eyes widen, eyes light up, grins, beams, gasps, laughs (as an action verb), chuckles, giggles, waves, shakes, brandishes, thrusts, holds up, lifts up, raises, points at camera, leans in, leans forward, jumps, bounces, spins, twirls, dances. Note: "presents" and "brings toward lens" are allowed ONLY when qualified as a slow micro-move that avoids motion blur — see rule 6. Also forbidden: cinematic (bare), epic, dramatic (as a genre descriptor), 4K, 8K, professional video, perfect, flawless, stunning, smooth motion, gimbal, studio lighting, slow motion, background score, music swells.
