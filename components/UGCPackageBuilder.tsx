...
import { type MusicMood, MUSIC_TRACKS } from '@/lib/music-library'
...
interface UGCPackageBuilderProps {
  ...
  externalPrefill?: { productName?: string; productDescription?: string; benefits?: string; callToAction?: string }
...
export default function UGCPackageBuilder({ onGenerate, isLoading, creditBalance, externalPrefill, initialActorId }: UGCPackageBuilderProps) {
  ...
  // Apply external prefill (e.g. from URL-to-ad scraper) when it changes.
  useEffect(() => {
    if (!externalPrefill) return
    if (externalPrefill.productName) setProductName(externalPrefill.productName.slice(0, 80))
    if (externalPrefill.productDescription) setProductDescription(externalPrefill.productDescription)
    if (externalPrefill.benefits) setBenefits(externalPrefill.benefits)
    if (externalPrefill.callToAction) setCallToAction(externalPrefill.callToAction)
  }, [externalPrefill])

  // Pre-select a saved actor when one is passed from the actor gallery.
  useEffect(() => {
    if (!initialActorId) return
    setSavedActorId(initialActorId)
    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <section ref={step4Ref} className="card step-reveal" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div className="form-row">
          ...
          <p className="help">Optional music track mixed under your video at low volume. Added during the stitch step.</p>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button
              type="button"
              onClick={() => setMusicMood(null)}
              style={{
                padding: '7px 14px', borderRadius: 8, fontSize: 12.5, fontWeight: 600, cursor: 'pointer',
                border: `1.5px solid ${musicMood === null ? 'var(--ink)' : 'var(--border)'}`,
                background: musicMood === null ? 'var(--ink)' : 'var(--surface)',
                color: musicMood === null ? 'var(--on-ink)' : 'var(--ink-2)',
              }}
            >
              None
            </button>
            {(Object.values(MUSIC_TRACKS) as Array<typeof MUSIC_TRACKS[MusicMood]>).map(track => (
              <button
                key={track.mood}
                type="button"
                onClick={() => setMusicMood(track.mood)}
                style={{
                  padding: '7px 14px', borderRadius: 8, fontSize: 12.5, fontWeight: 600, cursor: 'pointer',
                  border: `1.5px solid ${musicMood === track.mood ? 'var(--ink)' : 'var(--border)'}`,
                  background: musicMood === track.mood ? 'var(--ink)' : 'var(--surface)',
                  color: musicMood === track.mood ? 'var(--on-ink)' : 'var(--ink-2)',
